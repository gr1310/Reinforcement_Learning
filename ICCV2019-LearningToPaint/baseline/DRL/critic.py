...
# apply weight normalization to the convolution layer
...
# non linearity to the output of neurons in hidden layer
# ReLU(x)=(x)+=max(0,x)
...
# ResNet (Residual Network) is a deep convolutional neural network (CNN) architecture
# The ResNet architecture introduces the concept of residual blocks that use skip connections to connect the activations of a layer to further layers by skipping some layers in between. This forms a residual block. ResNets are made by stacking these residual blocks together

# return tuple (block class, [blobks used in first, second, third, fourth stage])
def cfg(depth):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        # a shortcut layer if the stride is not 1 or the input and output channels do not have the same dimensions
        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion*planes:
            # 1x1 kernel size
            self.shortcut = nn.Sequential(
